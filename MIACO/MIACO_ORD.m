 function [dd candidate] = MIACO_ORD(NCONV,NDISCV,K,SAI,q,rho, nAnt,Max_Iter, CONSTOL, TOL);
%  format long
%--------------------------------------------------------------------------
...
%     OFAS(1:nAnt) = feval(objfunc,SAFOROF_ANT(1:nAnt,1:NDIM));
    ...
    CPUTIME=cputime;
    [ObjFunAd res nA1 pAdsorbed]=objFun_amount_adsorbed(TSOL);
%%    dd(iter,:) = [iter Best_OF Iter_Best_OF TSOL];
    dd(iterrrr,:) = [iter Best_OF Iter_Best_OF TSOL nA1 pAdsorbed ObjFunAd];
...
%     disp(['Iter ' num2str(iter) ': Best So-far Cost = ' num2str(dd(iter,2)) ': Iter Best = ' num2str(dd(iter,3)) 'Dec Vars: X = ' num2str(dd(iter,4:NDIM+3)) ]); 
...
ylabel('Objective function value best-so-far vs iteration best');
xlabel('iterations');
...
% ylabel('Objective function value pen_f/obj_f');
% xlabel('iterations');
% 
% 
% subplot(2,2,3);
...
% ylabel('Residual, constraints (equality and inequality ');
% xlabel('iterations');
